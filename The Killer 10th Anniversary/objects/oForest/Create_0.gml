...
Location(DAY_SOUND, NIGHT_SOUND, 2);
locationType = "forest";
...
function createItemHere(){
	...
	if(random(1) > 0.2 and oPlayer.walking){
		...
		/*else if(creationNumber < 0.2){
			instance_create_depth(room_width+10, 175,0, oForestPalm);
		}*/
	...
	if (random_range(0,1) > 0.4 && global.playSounds){
		...
		if (oMyWorldController.time == "night"){
			if (creationNumber < 0.05 && !audio_is_playing(sndOwl)){
		...
		// Day sounds
			...
			else if (creationNumber < 0.015 && !audio_is_playing(sndOwl)){
				audio_emitter_gain(sEmit1, vol);
				audio_emitter_position(sEmit1, pan,0,0);
